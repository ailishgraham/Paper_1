...
circ_array = np.empty([n_circ,n_years,n_sites,n_days])
circ_array[:,:,:,:] = np.nan
n_stats = 5
n_seasons = 4
...
#np.save('circ_pm25_data_checked', circ_array, allow_pickle=True, fix_imports=True)
#print(circ_array[0,:,:,:])
#print(circ_array[3,:,:,:])
    ...
    for i_circ in range(len(circ_type)):
        for i_site in range(n_sites):
            ...
            if stop[i_season] == 365:
                d_flattened = np.ndarray.flatten(circ_array[i_circ,:, i_site, start[i_season]:stop[i_season]])
                d_flattened1 = np.ndarray.flatten(circ_array[i_circ,:, i_site, 0:59])
...
for i_circ in range(len(circ_type)): 
        ...
        data = circ_anomaly_annual[3,i_circ,:]
        print('data = ', data)
        time.sleep(2)
        xi,yi = map(lons,lats)
        #print 'lon, lat',  xi, yi
        print( 'creating plot -annual',circ_type[i_circ])
        sc = map.scatter(xi,yi,s=100,c=data,vmin=-15,vmax=15,cmap=cmap,norm=norm)#,cmap=plt.cm.jet)#marker='o',color='k')
        ax2 = fig.add_axes([0.95, 0.1, 0.03, 0.8])
        cb = mpl.colorbar.ColorbarBase(ax2, cmap=cmap, norm=norm, spacing='proportional', ticks=bounds, boundaries=bounds, format='%1i')
        cb.set_label('Anomaly', fontsize=20)
        cb.set_label('Anomaly', fontsize=20)
        plt.title(('Annual 95th percentile anomaly under '+str(circ_type[i_circ])+' circulation 2010-2016'),fontsize=20) 
        plt.savefig(('annual_95th_percentile_under_'+str(circ_type[i_circ])+'_circulation_anomaly.png'))
        plt.show()
        time.sleep(1)
