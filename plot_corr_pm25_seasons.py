...
filestem = '/nfs/see-fs-01_teaching/ee15amg/TRAJ_MODEL/AURN_data/complete/post_transfer/correl_data/binned_pm25_background_lats_winter_'
...
for ilwt in np.arange(len(lamb_weather_type)):
    ...
    print 'dfold',df.head()
    df = df.iloc[:,[0, 2, 5, 11, 12, 14, 15, 16, 18, 19, 21, 22, 25, 26, 27, 28, 31, 32, 33, 35, 39, 40, 42, 43]]
    print 'dfnew',df.head()
    ...
    plt.imshow(b, cmap='RdYlGn', interpolation='none', aspect='auto')
    plt.colorbar()
    plt.xticks(range(len(b)), b.columns, rotation='vertical')
    plt.yticks(range(len(b)), b.columns)
...
#    plt.yticks(range(len(c)), descending_lats)
    ...
    plt.title('Correlation of Winter PM$_{2.5}$ at sites during LWT - '+str(lamb_weather_type[ilwt]))
    plt.xlabel('Site')
    plt.ylabel('Site')
    plt.tight_layout()
    plt.savefig('correlate_pm25_winter_sites_gddata_all_'+str(lamb_weather_type[ilwt]))
    #plt.savefig('correlation_winter_pm25_sites_lwt_'+str(lamb_weather_type[ilwt]))
    plt.show()
...
#    plt.yticks(range(len(c)), descending_lats)
    ...
    plt.ylabel('Site')
    plt.tight_layout()
    plt.savefig('correlate_pm25_winter_sites_gddata_nona_'+str(lamb_weather_type[ilwt]))
    #plt.savefig('correlation_winter_pm25_sites_lwt_'+str(lamb_weather_type[ilwt]))
    plt.show()
