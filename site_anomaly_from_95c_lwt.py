...
# Calculate anomaly of stats of each lwt from stats of ll lwts          
...
##Create map  of pm2.5 concentrations at percentiles
        ##Change stats number i.e. statistics_annual[number,i_lwt,:] 
        ##to look at different stats
...
for i_lwt in range(len(lamb_weather_type)-1):
    ...
    data = statistics_annual[0,i_lwt,:]
    #print('data = ', data)
    #time.sleep(2)
    xi,yi = map(lons,lats)
    #print 'lon, lat',  xi, yi
    print( 'creating plot', i_lwt)
...
cb.set_label('Concentration', fontsize=15)
plt.savefig(('Annual_mean_lwt.png'))
plt.show()



#Create map of anomalies 
...
for i_lwt in range(len(lamb_weather_type)-1):
    ...
    data = lwt_anomaly_annual[3,i_lwt,:]
    #print('data = ', data)
    #time.sleep(2)
    xi,yi = map(lons,lats)
    #print 'lon, lat',  xi, yi
    print( 'creating plot', i_lwt)
    sc = map.scatter(xi,yi,s=50,c=data,vmin=-30,vmax=30,cmap=cmap)#,cmap=plt.cm.jet)#marker='o',color='k')
    ax.annotate(str(lamb_weather_type[i_lwt]), xy=(0.8, 0.8), xycoords="axes fraction",fontsize=15, fontweight='bold')
    fig.tight_layout()
...
plt.savefig(('Annual_90th_percentile_lwt_anomalies.png'))
plt.show()
...
for i_lwt in range(len(lamb_weather_type)-1):
    ...
    data = lwt_anomaly_annual[2,i_lwt,:]
    #print('data = ', data)
    #time.sleep(2)
    xi,yi = map(lons,lats)
    #print 'lon, lat',  xi, yi
...
cb.ax.tick_params(labelsize=font_size)
cb.set_label('Anomaly', fontsize=15)
plt.savefig(('Annual_75th_percentile_lwt_anomalies.png'))
plt.show()
