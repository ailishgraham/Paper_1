...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar  9 12:18:05 2018
...
#print( data_yearly_lwt[:,:],data_yearly_lwt[0,0],data_yearly_lwt[1,0])
#np.save('data_yearly_lwt_classification',data_yearly_lwt,allow_pickle=True, fix_imports=True)
#time.sleep(10)
...
statistics_annual = np.zeros([n_stats,n_circ,n_sites])
circ_anomaly_annual = np.zeros([n_stats,n_circ,n_sites])
...
#        #print 'lon, lat',  xi, yi
#        print( 'creating plot')
#        sc = map.scatter(xi,yi,s=100,c=data,vmin=-15,vmax=15,cmap=cmap,norm=norm)#,cmap=plt.cm.jet)#marker='o',color='k')
#        ax2 = fig.add_axes([0.95, 0.1, 0.03, 0.8])
#        cb = mpl.colorbar.ColorbarBase(ax2, cmap=cmap, norm=norm, spacing='proportional', ticks=bounds, boundaries=bounds, format='%1i')
#        cb.set_label('Anomaly', fontsize=20)
#        plt.title(str(seasons[i_season]+' 95th percentile anomaly under '+str(circ_type[i_circ])+' circulation 2010-2016' ),fontsize=20) 
#        plt.savefig(str(seasons[i_season]+'_95th_percentile_under_'+str(circ_type[i_circ])+'_circulation_anomaly.png'))
#        plt.show()
#        time.sleep(1)
            
        #time.sleep(5)

# Calculate and plot annual statistics

for i_circ in range(len(circ_type)):
    for i_site in range(n_sites):
        
        d_flattened = np.ndarray.flatten(lwt_array[i_circ,:, i_site, :])
        #print 'i_year', i_year, 
        #print('d flat', d_flattened)
        #print ('non-nans',(np.sum(~np.isnan(d_flattened))))
        #print('len_d', len(d_flattened))
        #print('% data available', (np.sum(~np.isnan(d_flattened))/len(d_flattened)*100))
        centile50 = np.nanpercentile(d_flattened, 50)
        statistics_annual[0,i_circ,i_site]= centile50
        #print('50=', centile50)
        #time.sleep(2)
        centile25 = np.nanpercentile(d_flattened, 25)
        statistics_annual[1,i_circ,i_site]= centile25
        #print('25=', centile25)
        #time.sleep(2)
        centile75 = np.nanpercentile(d_flattened, 75)
        statistics_annual[2,i_circ,i_site]= centile75
        #print('75=', centile75)
        #time.sleep(2)
        centile90 = np.nanpercentile(d_flattened, 90)
        statistics_annual[3,i_circ,i_site]= centile90
        #print('90=', centile90)
        #time.sleep(1)
        centile10 = np.nanpercentile(d_flattened, 10)
        statistics_annual[4,i_circ,i_site]= centile10
        #print('10=', centile10)
...
for i_stat in range(n_stats):    
    for i_circ in range(len(circ_type)):
        for i_site in range(n_sites):
            print('stat',i_stat,'circ',i_circ,'site',i_site)
            temp_data_anom = ((statistics_annual[i_stat,i_circ,i_site])-(statistics_annual[i_stat,3,i_site]))
            print('td=',temp_data_anom)
            #time.sleep(2)
            circ_anomaly_annual[i_stat,i_circ,i_site]=temp_data_anom
print('stat circ',statistics_annual[3,0,0],'stat circ all',statistics_annual[3,3,0], 'anom',circ_anomaly_annual[3,0,0])
...
x = range(n_sites)
y0 = circ_anomaly_annual[2,0,:]
y1 = circ_anomaly_annual[2,1,:]
y2 = circ_anomaly_annual[2,2,:]

ax.plot(x,y0,marker='o', color=marker_color[0], label=(circ_type[0]))
ax.plot(x,y1,marker='o', color=marker_color[1], label=(circ_type[1]))
ax.plot(x,y2,marker='o', color=marker_color[2], label=(circ_type[2]))
...
plt.title(('Annual 75th percentile anomaly under all circulation types 2010-2016'),fontsize=20) 
plt.savefig(('scatter_annual_75th_percentile_under_all_circulation_anomaly.png'))
plt.xlabel('Sites')
...
fig, ax = plt.subplots(figsize=(10, 10))
x = range(n_sites)
y0 = circ_anomaly_annual[0,0,:]
y1 = circ_anomaly_annual[0,1,:]
y2 = circ_anomaly_annual[0,2,:]
...
#label_lines = mpatches.Patch(color=marker_color[0], label=circ_type[0])
#plt.legend(handles=[label_lines])
plt.title(('Annual Mean anomaly under all circulation types 2010-2016'),fontsize=20) 
plt.savefig(('scatter_annual_mean_under_all_circulation_anomaly.png'))
plt.xlabel('Sites')
plt.ylabel('PM$_{2.5}$ Concentration')
plt.ylim(-8,9)
plt.show()
...
fig, ax = plt.subplots(figsize=(10, 10))
x = range(n_sites)
y0 = circ_anomaly_annual[3,0,:]
y1 = circ_anomaly_annual[3,1,:]
y2 = circ_anomaly_annual[3,2,:]
...
#label_lines = mpatches.Patch(color=marker_color[0], label=circ_type[0])
#plt.legend(handles=[label_lines])
plt.title(('Annual 90th percentile anomaly under AC,C and N circulation 2010-2016'),fontsize=20) 
plt.savefig(('scatter_annual_90th_percentile_under_all_circulations_anomaly.png'))
plt.xlabel('Sites')
plt.ylabel('PM$_{2.5}$ Concentration')
plt.ylim(-8,9)
plt.show()
